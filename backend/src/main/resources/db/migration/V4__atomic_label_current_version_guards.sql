...
BEGIN
  ...
  /*
   * product is the serialization point for:
   * - creation of a newer label draft
   * - current formula pointer changes
   *
   * JdbcLabelDraftRepository uses the same product row lock.
   */
  SELECT
      lv.product_id,
  ...
  IF v_permission_count < 1 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Actor lacks LABEL.SUBMIT_REVIEW permission';
  END IF;
  ...
  IF v_passed_validation_count < 1 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Passed validation is required before pending review';
  END IF;
  ...
  IF ROW_COUNT() <> 1 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Only DRAFT labels can move to PENDING_REVIEW';
  END IF;
...
BEGIN
  ...
  ELSE
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Unsupported label decision';
  END IF;

  START TRANSACTION;

  /*
   * Lock the same product serialization row before deciding whether this
   * label is still current.
   */
  SELECT
      lv.product_id,
      lv.formula_version_id,
  ...
  IF v_permission_count < 1 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Actor lacks required label-decision permission';
  END IF;

  SELECT rt.review_task_id
  INTO v_review_task_id
  FROM review_task rt
  WHERE rt.draft_label_version_id = p_label_version_id
    AND EXISTS (
      SELECT 1
      FROM label_version lv
      WHERE lv.label_version_id = p_label_version_id
  ...
  IF v_review_task_id IS NULL THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Decision requires a pending-review label and review task';
  END IF;

  IF v_creator_user_id = p_actor_user_id THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Maker-checker violation: label creator cannot approve own label';
  END IF;
  ...
  IF ROW_COUNT() <> 1 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Label decision state transition failed';
  END IF;
